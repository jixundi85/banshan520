...
const sampleCases = [
  ...
  },
  {
    ...
    coverImage: 'https://images.unsplash.com/photo-1536240478700-b869ad10e2f3?w=800&q=80',
    category: 'film',
    categoryName: 'AI电影',
...
const creatorProfile = {
  ...
  title: 'AIGC领域资深导师',
  avatar: '👩‍💻',
  avatarUrl: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Linxiaoyi',
...
export function populateSampleData() {
  try {
    // 不自动设置creatorProfile，避免覆盖真实用户数据
    ...
    // 保存教程数据（使用 savedTutorials 作为 key）
    const existingTutorials = JSON.parse(localStorage.getItem('savedTutorials') || '[]');
    const newTutorials = sampleTutorials.filter(
      newT => !existingTutorials.some(existing => existing.id === newT.id)
    );
    localStorage.setItem('savedTutorials', JSON.stringify([...existingTutorials, ...newTutorials]));
...
// 导出数据供其他组件使用
export { sampleCases, sampleTutorials, creatorProfile };
